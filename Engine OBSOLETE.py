	...
	def moveplayer(self):
		...
		if len(collisionbox["inst"]) > 0:
			if collisionbox["inst"][0].type == "slantr":
				groundslope = 45
			if collisionbox["inst"][0].type == "slantl":
				groundslope = -45


		self.println(groundslope,14)
				...
				if ground:
					...
					if not self.lastframejumped and not self.gp("lastframewall"):
						...
						if groundslope == 0:
							...
							self.print("!@#$%^&*()")
							mag = univars.func.dist([0,0],self.gp("act_vel"))
							magd = univars.func.dist([0,0],self.gp("des_vel"))
							self.sp("des_vel",[  mag * math.cos(groundslope)    ,    mag * math.sin(groundslope)   ])
							self.sp("act_vel",[  magd * math.cos(groundslope)    ,    magd * math.sin(groundslope)   ])

							if not self.lastgroundslope == 0:
								om.objects["player"]["pos"][0] = instlist[0].realpos[0] + -20
								om.objects["player"]["pos"][1] = instlist[0].realpos[1] + 0
					else:
						self.lastframejumped = 0
		...
		if not (collision["topmid"]["inst"] and collision["botmid"]["inst"] and collision["midright"]["inst"] and collision["midleft"]["inst"] ) or rail:
			...
			actlen = univars.func.dist([0,0],self.gp("act_vel"))

			relactvel = [  math.cos(groundslope)*actlen    ,  self.gp("act_vel",1) + math.sin(groundslope)*actlen   ]
		...
		self.lastgroundslope = groundslope
